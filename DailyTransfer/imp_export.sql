...
BEGIN
	...
	select e.*,PrepaidFtyImportFee2 = iif(isnull(e.MainWKID, '')<>'', MainWKID.PrepaidFtyImportFee2, MainExportID08.PrepaidFtyImportFee2) into #TExport from Trade_To_Pms.dbo.Export e WITH (NOLOCK)
	outer apply(
		select PrepaidFtyImportFee2=e2.PrepaidFtyImportFee
		from Trade_To_Pms.dbo.Export e2
		where e.MainWKID  = e2.id
	)MainWKID 
	outer apply(
		select PrepaidFtyImportFee2=e2.PrepaidFtyImportFee
		from Trade_To_Pms.dbo.Export e2
