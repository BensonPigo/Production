...
AS
BEGIN
	DECLARE @Physical AS VARCHAR(10);
	...
	DECLARE @BrandIDLLL AS VARCHAR(16);
	select @BrandIDLLL = Physical from FIR where id = @FirID


	if @BrandIDLLL = 'LLL G5'
	begin
		...
		BEGIN
			...
			SELECT @Physical = F.Physical
					...
					IF @Weight = 'Fail'	return	'Fail EXC LLL G5';
					IF @Continuity = 'Fail' return	'Fail EXC LLL G5';
					IF @ShadeBond = 'Fail'return	'Fail EXC LLL G5';
					IF @Odor = 'Fail'return	'Fail EXC LLL G5';
					IF @Moisture = ''return	'';
					IF @Moisture = 'Fail'return	'Fail EXC LLL G5';
