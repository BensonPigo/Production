...
CREATE FUNCTION [dbo].[GetOrderStatusByPOCombo]
(
	@POID varchar(13)
)
...
Begin
	...
	IF exists(Select Top 1 1 From Orders with(nolock) Where ID = @POID And IsBuyBack = 1)
	Begin 
		-- Buy Back
		Set @Status = 'Buy Back'
	End 
			...
			From (
				...
				Outer apply (
					Select value = stuff(( 
							...
							Where do.POID = o.POID And do.NeedProduction = 1 And do.Junk = 1
							Order by do.ID
							For xml path('')
							), 2, 13)
						), 1, 0, '')
				...
				Outer apply (
					Select value = stuff(( 
							o.POID + SubString((Select '/' + REPLACE(do.ID, @poid, '') 
							From dbo.Orders do with(nolock)
							Where do.POID = o.POID And do.KeepPanels = 1 And do.Junk = 1
							Order by do.ID
							For xml path('')
							), 2, 13)
						), 1, 0, '')
				...
				Outer apply (
					Select value = stuff(( 
							o.POID + SubString((Select '/' + REPLACE(do.ID, @poid, '') 
							From dbo.Orders do with(nolock)
							Where do.POID = o.POID And do.NeedProduction = 0 And do.KeepPanels = 0 And do.Junk = 1
							Order by do.ID
							For xml path('')
							), 2, 13)
						), 1, 0, '')
