...
DECLARE @fabric_detaildesc nvarchar(max); -- 暫存fabric description
	...
	BEGIN
		...
		BEGIN	
			select @fabric_detaildesc= ISNULL(DescDetail,'') from fabric WITH (NOLOCK) where SCIRefno = @scirefno;
			set @string = concat(rtrim(iif(isnull(@fabric_detaildesc, '') = '','',@fabric_detaildesc))
								, CHAR(13), CHAR(10)
								, rtrim(iif(isnull(@suppcolor, '') = '','',@suppcolor))
								, CHAR(13), CHAR(10)
								, rtrim(iif(isnull(@po_desc, '') = '','',@po_desc))
								, CHAR(13), CHAR(10));
		END
		ELSE
			set @string = rtrim(iif(isnull(@suppcolor, '') = '','',@suppcolor + CHAR(13)+ CHAR(10)))+rtrim(iif(isnull(@po_desc, '') = '','',@po_desc+CHAR(13)+ CHAR(10)));
	...
	BEGIN	
		...
		End
		ELSE
			set @string = rtrim(iif(isnull(@suppcolor, '') = '','',@suppcolor + CHAR(13)))+rtrim(iif(isnull(@po_desc, '') = '','',@po_desc+CHAR(13)+ CHAR(10)));
	END
