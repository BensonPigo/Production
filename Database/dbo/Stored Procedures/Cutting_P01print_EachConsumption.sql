...
BEGIN
	...
	FROM dbo.Orders
	OUTER APPLY(SELECT STUFF((SELECT '/'+SUBSTRING(ID,11,4) FROM Production.dbo.Orders WHERE POID = @OrderID  order by ID FOR XML PATH('')),1,1,'') as spno
	...
	inner join dbo.Order_EachCons_SizeQty b on a.Id = b.Id and a.Ukey = b.Order_EachConsUkey
	inner join dbo.Order_EachCons_Color c on a.Id = c.Id and a.Ukey = c.Order_EachConsUkey
