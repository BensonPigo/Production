...
CREATE PROCEDURE [dbo].[Cutting_Report_QtyBreakdown]
	@OrderID varchar(13)
	,@ByType int = 0 --0單張 , 1 By OrderCombo , 2 By PO
	,@ByKit bit = 0 -- 1:顯示by Kit結果
	,@ByStatus int = 0 --0:全印, 1:NeedProduction = 1, 2:KeepPanel = 1, 3:BuyBack = 1, 4:Junk = 1
...
where b.ID is not null and (@ByKit = 0 or (@ByKit = 1 and e.kit != '' and e.Kit is not null))
	and (@ByStatus = 0 
		or (@ByStatus = 1 and Isnull(d.NeedProduction, 0) = 1)
		or (@ByStatus = 2 and Isnull(d.KeepPanels, 0) = 1)
		or (@ByStatus = 3 and Isnull(d.IsBuyBack, 0) = 1)
		or (@ByStatus = 4 and Isnull(d.Junk, 0) = 1))
...
if exists(select 1 from #tmp_col)
	begin
		declare @str1 nvarchar(max),@str2 nvarchar(max),@str3 nvarchar(max),@str4 nvarchar(max),@strKit nvarchar(10) = '',@str5 nvarchar(10) = ''
