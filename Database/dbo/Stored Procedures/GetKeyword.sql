...
Create Procedure [dbo].[GetKeyword]
	(
	 ...
	 ,@Keyword				VarChar(Max)
	 ,@Keyword_AfterTrans	VarChar(Max)	Output
	 ,@Article				VarChar(8)	= ''
	 ,@SizeCode				VarChar(8)	= ''
	)
As
...
SET QUOTED_IDENTIFIER ON
	...
	Declare @tmp_Keyword Table
		(RowID BigInt Identity(1,1) Not Null, KeywordID VarChar(30), KeyValue VarChar(Max));
	...
	Begin
		...
		Begin
			...
			Begin
				...
				Begin
					Set @KeyValue = dbo.GetColorName(@BrandID, @KeyValue, '3', '');
				End;
			End;
			...
			End;
			/*
			If IsNull(@KeyValue, '') = '' And Upper(@FieldTable) = 'Orders'
			Begin
			...
			*/
			If IsNull(@KeyValue, '') = ''
			Begin
				Exec dbo.GetKeyWordValue @OrderID, @KeywordID, @KeyValue Output;
			End;
		End;
