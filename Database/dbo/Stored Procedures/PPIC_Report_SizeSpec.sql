...
BEGIN
	...
	SELECT distinct a.SizeCode,Seq into #tmp_Col FROM Order_SizeCode a inner join Order_Qty b on a.Id = b.ID and (@fullsize = 1 or a.SizeCode = b.SizeCode)
	where a.Id = @POID
	...
	if exists(select 1 from #tmp_Col)
		begin
			...
			declare @sql nvarchar(max) = 'select SizeItem,[Description          Unit : '+ @Unit +']=Description,'+ @str1 +' from (
				select a.SizeItem,a.Description,b.SizeCode,b.SizeSpec from Trade.dbo.Order_SizeItem a
				left join trade.dbo.Order_SizeSpec b on a.Id = b.Id and a.SizeItem = b.SizeItem
