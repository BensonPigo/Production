...
-- =============================================
CREATE PROCEDURE [dbo].[imp_Style] 
...
from Trade_To_Pms.dbo.Style_TmsCost as b WITH (NOLOCK)
where not exists(select 1 from Production.dbo.Style_TmsCost as a WITH (NOLOCK) where a.StyleUkey=b.StyleUkey AND a.ArtworkTypeID	= b.ArtworkTypeID)
...
-----------------Style_MarkerList_Article-------------------
	...
	when not matched by target then
		...
		values(s.StyleUkey,
				...
				s.EditName,
				s.EditDate)
	when not matched by source and t.styleUkey in (select distinct styleUkey from production.dbo.style_markerlist WITH (NOLOCK)) then
		delete;
...
,EditName
,EditDate
from Trade_To_Pms.dbo.Style_ColorCombo as b WITH (NOLOCK)
