...
Begin
	...
	Begin
		...
		----取出TransferLocation需要的PackingList_Detail即可，後續需要查詢的話就用這個範圍內的資料
		select pd.*
		into #TransferLocation_PackingList_Detail
		from Production.dbo.PackingList_Detail pd
		inner join TransferLocation s on pd.SCICtnNo=s.SCICtnNo
	...
	Begin
		...
		Begin
			...
			select distinct [TransferDate] = CONVERT(date, t.Time)
				...
				,pd.SCICtnNo
			from #tmpTransferLocation t
			inner join #TransferLocation_PackingList_Detail pd on t.SCICtnNo=pd.SCICtnNo
	...
	Begin		
		...
		and (
			(exists (select 1
					from  #TransferLocation_PackingList_Detail  pd
		...
		from #TransferLocation_PackingList_Detail as pd
		inner join #tmp_LastLocation s on pd.SCICtnNo = s.SCICtnNo
		where row=1
		...
		where exists(
			...
			inner join #TransferLocation_PackingList_Detail pd 
