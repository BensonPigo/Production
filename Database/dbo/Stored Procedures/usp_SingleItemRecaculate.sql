...
BEGIN
    ...
    BEGIN TRY
				...
				BEGIN
					...
					VALUES (@MDivisionid,@Poid,@Seq1,@seq2)
					SELECT @Ukey = ukey from @NewUkey;
				END
				
				-- FtyInventory 資料歸零
				SET @SQLCMD = N'UPDATE FtyInventory set inqty = 0,outqty = 0 ,adjustqty = 0
					where MDivisionid = @pMDivisionid and POID = @pPoid and Seq1 = @pSeq1 and Seq2 = @pSeq2';

				EXEC sp_executesql @SQLCMD , N'@pMDivisionid varchar(8), @pPoid varchar(13) ,@pSeq1 varchar(3), @pSeq2 varchar(2)'
				,@pMDivisionid = @MDivisionid, @pPoid = @Poid ,@pSeq1 = @Seq1, @pSeq2 = @Seq2;
				...
				USING (SELECT g.*,
						...
						FROM group6key g) AS S --(MDivisionid,poid,seq1,seq2,roll,dyelot,AInqty,AOutqty,AAdjustQty)
				ON T.MDIVISIONID = S.MDivisionid AND T.POID = S.POID AND T.SEQ1 =  S.SEQ1 AND T.SEQ2 = S.SEQ2 AND T.ROLL = S.ROLL and T.Dyelot = S.Dyelot and t.stocktype='B'
				WHEN MATCHED THEN UPDATE set inqty = S.AInqty, outqty = s.AOutqty , adjustQty = AAdjustqty
				WHEN NOT MATCHED BY TARGET THEN 
				INSERT (MDivisionPoDetailUkey,MdivisionID,poid,seq1,seq2,roll,stocktype,dyelot,inqty,outqty,adjustqty)
				values (@Ukey,S.MDivisionid,S.POID,S.SEQ1,S.SEQ2,S.ROLL,'B',S.dyelot,S.AInqty,S.AOutqty,S.AAdjustQty)
				...
				USING (SELECT g.*,
						Dyelot = (select top 1 gd.Dyelot from groupDyelot gd where g.Roll = gd.Roll and g.seq1 = gd.seq1 and g.seq2 = gd.seq2 and g.PoId = gd.PoId and g.MDivisionID = gd.MDivisionID)
						FROM group6key g) AS S --(MDivisionid,poid,seq1,seq2,roll,dyelot,BInqty,BOutqty,BAdjustQty)
				ON T.MDIVISIONID = S.MDivisionid AND T.POID = S.POID AND T.SEQ1 =  S.SEQ1 AND T.SEQ2 = S.SEQ2 AND T.ROLL = S.ROLL and T.Dyelot = S.Dyelot and t.stocktype='I'
				WHEN MATCHED THEN UPDATE set inqty = S.BInqty, outqty = s.BOutqty , adjustQty = BAdjustQty
				WHEN NOT MATCHED BY TARGET THEN 
				INSERT (MDivisionPoDetailUkey,MdivisionID,poid,seq1,seq2,roll,stocktype,dyelot,inqty,outqty,adjustqty)
				values (@Ukey,S.MDivisionid,S.POID,S.SEQ1,S.SEQ2,S.ROLL,'I',S.dyelot,S.BInqty,S.BOutqty,S.BAdjustQty)
				...
				USING (SELECT g.*,
						Dyelot = (select top 1 gd.Dyelot from groupDyelot gd where g.Roll = gd.Roll and g.seq1 = gd.seq1 and g.seq2 = gd.seq2 and g.PoId = gd.PoId and g.MDivisionID = gd.MDivisionID)
						FROM group6key g) AS S --(MDivisionid,poid,seq1,seq2,roll,dyelot,CInqty,COutqty,CAdjustQty)
				ON T.MDIVISIONID = S.MDivisionid AND T.POID = S.POID AND T.SEQ1 =  S.SEQ1 AND T.SEQ2 = S.SEQ2 AND T.ROLL = S.ROLL and T.Dyelot = S.Dyelot and t.stocktype='O'
				WHEN MATCHED THEN UPDATE set inqty = S.CInqty, outqty = s.COutqty , adjustQty = CAdjustQty
				WHEN NOT MATCHED BY TARGET THEN 
				INSERT (MDivisionPoDetailUkey,MdivisionID,poid,seq1,seq2,roll,stocktype,dyelot,inqty,outqty,adjustqty)
				values (@Ukey,S.MDivisionid,S.POID,S.SEQ1,S.SEQ2,S.ROLL,'O',S.dyelot,S.CInqty,S.COutqty,S.CAdjustQty)
				...
				RAISERROR ('%s-%s-%s-%s be caculated on %s', 1,10, @MDivisionid, @Poid,@Seq1,@Seq2,@currentDT);
