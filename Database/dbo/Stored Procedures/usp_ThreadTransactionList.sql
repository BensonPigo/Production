...
BEGIN
	...
	a.editname+'-'+c.name +' '+convert(varchar, a.editdate,120) as editname ,a.editdate
		From ThreadIncoming a left join Pass1 c on c.id=a.editname , ThreadIncoming_Detail b  
	...
	d.editname+'-'+f.name +' '+convert(varchar,d.editdate,120) as editname ,d.editdate
		From ThreadIssue d left join Pass1 f on f.id=d.editname , ThreadIssue_Detail e  
			Where e.Refno =@Refno and d.id = e.id  and status = 'Confirmed' and MDivisionid=@MDivisionid and cdate > = @Date1 and cdate < = @Date2 
	...
	a.editname+'-'+c.name +' '+convert(varchar,a.editdate,120) as editname,a.EditDate
		From ThreadTransfer a left join Pass1 c on c.id=a.editname , ThreadTransfer_Detail b  
			Where b.Refno =@Refno and a.id = b.id  and status = 'Confirmed' and MDivisionid=@MDivisionid and cdate > = @Date1 and cdate < = @Date2
