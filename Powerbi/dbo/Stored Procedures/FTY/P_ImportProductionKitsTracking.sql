...
SET NOCOUNT ON
	...
	where 1=1
	AND (@FirstDate IS NULL OR sp.SCIDelivery >= @FirstDate)
	AND
	(
	...
	inner join #tmp t on t.Article = p.Article and t.FactoryID = p.FactoryID and t.Doc = p.Doc and t.SPNo = p.SPNo
	where t.Article is null and  t.FactoryID is null and t.Doc is null and t.SPNo is null
	'
	set @sqlcmdUpdate = '
	UPDATE p SET
	...
	,isnull(t.FtyHandle,'''')
	FROM #tmp t 
	LEFT join P_ProductionKitsTracking p on t.Article = p.Article and t.FactoryID = p.FactoryID and t.Doc = p.Doc and t.SPNo = p.SPNo
	where p.Article is null and  p.FactoryID is null and p.Doc is null and p.SPNo is null
