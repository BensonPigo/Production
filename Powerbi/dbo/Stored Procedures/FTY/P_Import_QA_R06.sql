...
(
	...
	left join [MainServer].Production.dbo.View_WH_Orders v with (nolock) on r.PoId = v.ID
	where 1=1
...
outer apply(
	...
	from (
		...
		from Production.dbo.SplitString((select Defect from #spr WHERE SuppID = gbs.SuppID AND Refno=gbs.Refno 
...
from(
	...
	where  rd1.WhseArrival >= @WhseArrival_s and rd1.WhseArrival <= @WhseArrival_e
