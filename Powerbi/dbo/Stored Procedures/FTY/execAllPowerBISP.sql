...
/****************************************************************************************************************************/
/***********************************P_Import_CartonStatusTrackingList****************************************************************/
BEGIN TRY
	set @StartDate = DATEADD(MONTH,-1,GETDATE())
	set @Stime = getdate()  
	execute [dbo].[P_Import_CartonStatusTrackingList] @StartDate
	set @Etime = getdate()
END TRY
...
SET @ErrorMessage = 
'
[35-P_Import_CartonStatusTrackingList]' + CHAR(13) +
',錯誤代碼: ' + CONVERT(VARCHAR, ERROR_NUMBER()) + CHAR(13) +
',錯誤行數: ' + CONVERT(VARCHAR, ERROR_LINE()) + CHAR(13) +
',錯誤訊息: ' + ERROR_MESSAGE()
...
BEGIN 
	set @desc += CHAR(13) + '
[35-P_Import_CartonStatusTrackingList] is completed' + ' Time:' + FORMAT(@Stime, 'yyyy/MM/dd HH:mm:ss') + ' - ' + FORMAT(@Etime, 'yyyy/MM/dd HH:mm:ss')
END
ELSE
BEGIN
	set @desc += CHAR(13) + @ErrorMessage
...
-- Write in P_TransLog
	insert into P_TransLog(functionName,Description,StartTime,EndTime,TransCode) 
	values('P_Import_CartonStatusTrackingList',@ErrDesc,@Stime,@Etime,@TransCode)
