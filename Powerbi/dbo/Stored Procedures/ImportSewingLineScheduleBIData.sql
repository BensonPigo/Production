...
select
	...
	[CPU] = cast(o.CPU * o.CPUFactor * isnull(Production.dbo.GetOrderLocation_Rate(s.OrderID,s.ComboType),isnull(Production.dbo.GetStyleLocation_Rate(o.StyleUkey,s.ComboType),100)) / 100 as float),
	s.TotalSewingTime,
	s.OrderID,
	s.LNCSERIALNumber,
    s.ComboType
...
inner join Production.dbo.Factory f with (nolock) on f.id = s.FactoryID and Type <> 'S'
left join Production.dbo.Country c WITH (NOLOCK) on o.Dest = c.ID
...
from @APSList aps
inner join Production.dbo.SewingSchedule s with (nolock) on aps.APSNo = s.APSNo
inner join Production.dbo.Orders o with (nolock) on s.OrderID = o.ID
group by aps.APSNo
...
from @APSList aps
inner join Production.dbo.SewingSchedule s with (nolock) on aps.APSNo = s.APSNo
inner join Production.dbo.CuttingOutput_WIP cw with (nolock) on s.OrderID = cw.OrderID
group by aps.APSNo
...
from @APSList aps
inner join Production.dbo.SewingSchedule s with (nolock) on aps.APSNo = s.APSNo
inner join Production.dbo.PackingList_Detail pld with (nolock) on s.OrderID = pld.OrderID and pld.ReceiveDate is not null
group by aps.APSNo
...
from @APSList aps
inner join Production.dbo.SewingSchedule s with (nolock) on aps.APSNo = s.APSNo
inner join Production.dbo.SewingOutput_Detail sod with (nolock) on s.OrderID = sod.OrderID and s.ComboType = sod.ComboType
inner join Production.dbo.SewingOutput so with (nolock) on so.ID = sod.ID and s.SewingLineID = so.SewingLineID
group by	aps.APSNo,
			so.OutputDate
...
select  distinct
        ...
        , ot.TMS
        , at.Classify
from Production.dbo.Order_TmsCost ot WITH (NOLOCK) 
inner join Production.dbo.ArtworkType at WITH (NOLOCK)  on ot.ArtworkTypeID = at.ID
inner join Production.dbo.Orders o with (nolock) on ot.ID = o.id
where   (ot.Price > 0 or at.Classify in ('O','I') )
        and (at.Classify in ('S','I') or at.IsSubprocess = 1)
        and (ot.TMS > 0 or ot.Qty > 0)
        and at.Abbreviation !=''
		and ot.ID in (select OrderID from Production.dbo.SewingSchedule where exists(select 1 from @APSList where APSNo = SewingSchedule.APSNo)) 
...
from Production.dbo.SewingSchedule s with (nolock)
inner join Production.dbo.SewingSchedule_Detail sd with (nolock) on s.ID = sd.ID
...
from Production.dbo.SewingSchedule s with (nolock)
inner join Production.dbo.SewingSchedule s_other on s_other.OrderID = s.OrderID and s_other.ComboType = s.ComboType and s_other.APSNo <> s.APSNo
where  exists( select 1 from @APSList where APSNo = s.APSNo)
...
inner join Production.dbo.Workhour_Detail wkd with (nolock) on wkd.FactoryID = al.FactoryID and 
...
left join Production.dbo.LearnCurve_Detail lcd with (nolock) on awd.LearnCurveID = lcd.ID and awd.WorkDateSer = lcd.Day
left join @APSSewingOutput apo on awd.APSNo = apo.APSNo and awd.WorkDate = apo.OutputDate
outer apply(select top 1 [val] = Efficiency from Production.dbo.LearnCurve_Detail where ID = awd.LearnCurveID order by Day desc ) LastEff
group by awd.APSNo,
		 awd.SewingStart,
		 awd.SewingEnd,
...
--組合最終table
--先刪除全部 
Delete SewingLineScheduleBI;


--注意，Store Procedure 產生出來的欄位順序，必須與下列INSERT的順序一致
INSERT INTO [dbo].[SewingLineScheduleBI]
           ([APSNo]
           ,[SewingLineID]
           ,[Sewer]
           ,[SewingDay]
...
(
	select
		...
		[CPU]=apf.CPU,
		[SewingCPU] = ( apm.TotalSewingTime / (SELECT StdTMS * 1.0 FROm Production.dbo.System))  ,
