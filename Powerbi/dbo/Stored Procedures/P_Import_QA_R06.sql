...
';
	...
	outer apply(
		...
		from (
			...
			from dbo.SplitString((select Defect from #spr s WHERE s.SuppID = gbs.SuppID and s.Refno=gbs.Refno and s.FactoryID=gbs.FactoryID and s.WhseArrival=gbs.WhseArrival),''/'') x
		)A 
...
';
	...
	select a.SuppID,a.Refno,a.FactoryID,a.WhseArrival ,count(*)*1.0 Dcnt
	 ...
	 select a.SuppID ,a.Refno ,a.FactoryID ,a.WhseArrival
		, iif(isnull(b.Dcnt,0)=0, 0, round(b.Dcnt/a.Mcnt ,4)) [cnt]
	 into #BulkDyelot
	 from #tmp_DyelotMcnt a
	 left join #tmp_DyelotDcnt b on a.SuppID = b.SuppID and a.Refno=b.Refno and a.FactoryID=b.FactoryID and a.WhseArrival=b.WhseArrival
