...
namespace Sci.Production.Centralized
{
    ...
    public partial class IE_B92 : Win.Tems.Input1
    {
        ...
        protected override bool ClickSaveBefore()
        {
            ...
            {
                if (result = DBProxy.Current.Select("ProductionTPE", "select max_id = max(id) from IEReason  WITH (NOLOCK) WHERE  Type = 'LM'", out dt))
                {
            ...
            result = DBProxy.Current.Select("ProductionTPE", "select ID from IEReason  WITH (NOLOCK) WHERE Type = 'LM' AND Description = @Description", parameters, out dt);
