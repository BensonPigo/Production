...
namespace Sci.Production.Centralized
{
    ...
    internal partial class R13 : Sci.Win.Tems.PrintForm
    {
        ...
        private DataTable dt_detail_detail;
        private DataTable dt_detail_detail_All;
        private string tsql_Detail;
        private string tsql_Detail_Detail;
        ...
        protected override bool ValidateInput()
        {
            ...
            {
                if (this.dateOnboardDate1.Value.Empty())
                {
                    MyUtility.Msg.ErrorBox("[Begin On Board Date] can not  be empty");
                    this.dateOnboardDate1.Focus();
                }

                if (this.dateOnboardDate2.Value.Empty())
                {
                    MyUtility.Msg.ErrorBox("[End On Board Date] can not  be empty");
                    this.dateOnboardDate2.Focus();
                }
            ...
            if (!this.datePulloutDate1.Value.Empty() && !this.datePulloutDate2.Value.Empty())
            {
                sqlWheres.Add(string.Format("Pullout_Detail.PulloutDate between '{0}' and '{1}' ", ((DateTime)this.datePulloutDate1.Value).ToShortDateString(), ((DateTime)this.datePulloutDate2.Value).ToShortDateString()));
            ...
            if (!this.dateApproveDate1.Value.Empty() && !this.dateApproveDate2.Value.Empty())
            {
                sqlWheres.Add(string.Format("ShippingAP.ApvDate between '{0}' and '{1}' ", ((DateTime)this.dateApproveDate1.Value).ToShortDateString(), ((DateTime)this.dateApproveDate2.Value).ToShortDateString()));
            ...
            if (!this.dateOnboardDate1.Value.Empty() && !this.dateOnboardDate2.Value.Empty())
            {
                sqlWheres.Add(string.Format("GMTBooking.ETD between '{0}' and '{1}' ", ((DateTime)this.dateOnboardDate1.Value).ToShortDateString(), ((DateTime)this.dateOnboardDate2.Value).ToShortDateString()));
            ...
            if (this.checkExportDetail.Checked == true)
            {
                ...
                this.Transportation_Detail_Detail(sqlWhere);
            }
            else
            {
                this.Transportation_list(sqlWhere);
...
where ShippingAP.Type = 'EXPORT' and ShippingAP.subType <> 'SISTER FACTORY TRANSFER' and ",
                this.comboRateTypeID.Text) + sqlWhere +
 @"
select * 
into #temp3 
from (
...
order by ID,OrderID";
        ...
        protected override Ict.DualResult OnAsyncDataLoad(Win.ReportEventArgs e)
        {
            ...
            this.dt_detail_detail = null;
            this.dt_detail_detail_All = null;
            DualResult result;
...
";
                    ...
                    if (this.checkExportDetail.Checked == true)
                    {
                        ...
                        DualResult result3 = DBProxy.Current.SelectByConn(conn, this.tsql_Detail_Detail, out this.dt_detail_detail);
                        if (!result3)
                        {
                            return result3;
                        }

                        if (this.dt_detail_detail_All == null || this.dt_detail_detail_All.Rows.Count == 0)
                        {
                            this.dt_detail_detail_All = this.dt_detail_detail;
                        }
                        else
                        {
                            this.dt_detail_detail_All.Merge(this.dt_detail_detail); // 如果dt_detail_All有資料時，將資料Merge上去
                        }
                    }
                }
            }
...
";
        ...
        protected override bool OnToExcel(Win.ReportDefinition report)
        {
            ...
            if (this.checkExportDetail.Checked == true)
            {
                ...
                if ((this.dt_detail_detail_All == null || this.dt_detail_detail_All.Rows.Count == 0) && (this.dt_detail_All == null || this.dt_detail_All.Rows.Count == 0))
                {
                    MyUtility.Msg.ErrorBox("Data not found");
                    return false;
                }
            ...
            if (this.checkExportDetail.Checked == true)
            {
                ...
                SaveXltReportCls.XltRptTable xdt_detail_detail_All = new SaveXltReportCls.XltRptTable(this.dt_detail_detail_All);
        ...
        private void DateOnboardDate1_Validated(object sender, EventArgs e)
        {
            if (!this.dateOnboardDate1.Value.Empty())
            {
                this.dateOnboardDate2.Value = Convert.ToDateTime(this.dateOnboardDate1.Value).GetLastDayOfMonth();
            }
        }
        #endregion
    }
