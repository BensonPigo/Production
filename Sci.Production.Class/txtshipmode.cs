        ...
        public string UseFunction
        {
            set 
            {
                ...
                if (!Env.DesignTime)
                {
                    ...
                    {
                        sqlCMD = "select ID,UseFunction from ShipMode WITH (NOLOCK) order by ID";
                    }
                    else
                    {
                        sqlCMD = string.Format("select ID,UseFunction from ShipMode WITH (NOLOCK) where UseFunction like '%{0}%'", useFunction.Trim());
                    }
