...
namespace Sci.Production.Cutting
{
    ...
    public partial class P10_1_Print : Win.Tems.PrintForm
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="P10_1_Print"/> class.
        /// </summary>
        /// <param name="row">DataRow</param>
        public P10_1_Print(DataRow row)
        {
...
from (
    select a.BundleGroup [Group_right]
        ...
        ,b.item
        ,b.Remake
        ,NoBundleCardAfterSubprocess=(select top 1 N'(X)' from BundleReplacement_Detail_Art bda with(nolock) where bda.Bundleno = a.Bundleno and bda.NoBundleCardAfterSubprocess = 1)
    from dbo.BundleReplacement_Detail a WITH (NOLOCK) 
    left join dbo.BundleReplacement b WITH (NOLOCK) on a.id=b.id
    ...
    (
	    ...
	    (
		    ...
		    from dbo.BundleReplacement_Detail_Art e1 WITH (NOLOCK) 
    ...
    left join dbo.BundleReplacement b WITH (NOLOCK) on a.id=b.id
    left join dbo.orders c WITH (NOLOCK) on c.id=b.Orderid
    left join dbo.BundleReplacement_Detail_Allpart d WITH (NOLOCK) on d.id=a.Id
            ...
            if (this.radioBundleCard.Checked)
            {
                ...
                List<P10_PrintData> data = dt1.AsEnumerable()
                    .Select(row1 => new P10_PrintData()
                    {
                        ...
                        EXCESS1 = MyUtility.Convert.GetBool(row1["Remake"]) ? "Remake" : string.Empty,
                ...
                data.AddRange(
                 dt2.AsEnumerable().Select(row1 => new P10_PrintData()
                 {
                     ...
                     EXCESS2 = MyUtility.Convert.GetBool(row1["Remake"]) ? "Remake" : string.Empty,
                ...
                dt3.AsEnumerable().Select(row1 => new P10_PrintData()
                {
                    ...
                    EXCESS3 = MyUtility.Convert.GetBool(row1["Remake"]) ? "Remake" : string.Empty,
