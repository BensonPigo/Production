...
[Remark] = wo.UnfinishedCuttingRemark,
WOFP. cutplanID,
wo.MarkerVersion,
wo.Ukey,
...
LEFT JOIN WorkOrderForPlanning WOFP WITH(NOLOCK) ON WOFP.Ukey = WO.WorkOrderForPlanningUkey
...
outer apply(select AccuCuttingLayer = sum(aa.Layer) from cuttingoutput_Detail aa WITH (NOLOCK) where aa.WorkOrderForOutputUkey = wo.Ukey)acc
outer apply(
...
Where cod.WorkOrderForOutputukey = wo.Ukey and co.Status != 'New' 
) as MincDate
outer apply(
select SewInLine=(
	select SewInLine 
