...
namespace Sci.Production.Cutting
{
    public partial class R07 : Win.Tems.PrintForm
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="R07"/> class.
        /// </summary>
...
into #tmp2
from #tmp2a t
inner join WorkOrderForOutput w with(nolock) on w.CutRef = t.CutRef and w.id = t.id
...
outer apply(
	select SubSP = stuff((
		...
		where w2.CutRef = t.CutRef and w2.id = t.id
		For XML path('')
	),1,1,'')
)subSp
outer apply(
...
(
	...
	(
		Select concat(', ' , wd.sizecode, '/ ', wd.qty)
		From WorkOrderForOutput_SizeRatio wd WITH (NOLOCK)
		Where wd.WorkOrderForOutputUkey = t.Ukey
		For XML path('')
	),1,1,'')
)SizeCode
...
";
        ...
        protected override bool OnToExcel(ReportDefinition report)
        {
            ...
            worksheet.Range[$"B15:{cCol}25"].Borders.Weight = 2; // 設定全框線
            ...
            for (int i = 0; i < s; i++)
            {
                ...
                worksheet.Cells[6, i + 2] = $"={col}5*C$2*F$2";
            ...
            {
                worksheet.Cells[15, i + 2] = codt.Rows[i]["CutCellid"];
                worksheet.Cells[16, i + 2] = codt.Rows[i]["CuttingMachDescription"];
                col = MyUtility.Excel.ConvertNumericToExcelColumn(i + 2);
                worksheet.Cells[17, i + 2] = this.WorkHoursDay;
                worksheet.Cells[18, i + 2] = $"={col}17*C$2*F$2";
                worksheet.Cells[19, i + 2] = codt.Rows[i]["AvgCutSpeedMperMin"];
                worksheet.Cells[20, i + 2] = codt.Rows[i]["TotalCuttingPerimeter"];
                worksheet.Cells[21, i + 2] = codt.Rows[i]["TotalCutMarkerQty"];
                worksheet.Cells[22, i + 2] = codt.Rows[i]["TotalCutFabricYardage"];
                worksheet.Cells[23, i + 2] = codt.Rows[i]["TotalCuttingTime_hrs"];
