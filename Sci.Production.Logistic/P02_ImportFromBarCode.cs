                ...
                if (!MyUtility.Check.Empty(dr.Filename))
                {
                    using (StreamReader reader = new StreamReader(dr.Fullfilename, System.Text.Encoding.UTF8))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            // 如果有資料重複就不再匯入重複的資料
                            findRow = this.grid2Data.Select(string.Format("PackingListID = '{0}' and CTNStartNo = '{1}' or SCICtnNo = '{2}'", sl[2].Substring(0, 13), sl[2].Substring(13).Trim(), sl[2].GetPackScanContent()));
                            if (findRow.Length == 0)
                            {
