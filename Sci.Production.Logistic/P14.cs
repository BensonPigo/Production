...
";
        ...
        private void GridStyleChange()
        {
            ...
            {
                if (!gridRow.Cells["Remark"].Value.ToString().ToUpper().Contains("SUCCESSFULLY TRANSFER"))
                {
...
where   p.MDivisionID = '{Env.User.Keyword}' and
        ...
        p.PLCtnTrToRgCodeDate is null and
...
union all
select  p.ID,[Remark] = 'This PL already transfer to shipping factory!!'
from PackingList p with (nolock)
where   p.ID in ({wherePackID}) and p.PLCtnTrToRgCodeDate is not null
";

            result = DBProxy.Current.Select(null, sqlCheckPackStatus, out dtCheckResult);
            ...
            {
                drSelected["Remark"] = $"Successfully transfer to {this.comboTransferTo.Text}!!";
                drSelected["Fail"] = 0;
            }
            ...
            var canUpdateRow = drSelecteds.Where(s => (int)s["Fail"] == 0);
            if (canUpdateRow.Any())
            {
                string canUpdatePackIDs = canUpdateRow.Select(s => $"'{s["ID"].ToString()}'").JoinToString(",");
                string updSql = $"update PackingList set PLCtnTrToRgCodeDate = GetDate() where ID in ({canUpdatePackIDs})";
...
from Factory F 
where F.Junk = 0 and F.IsProduceFty = 1 and F.NegoRegion='{this.comboTransferTo.Text}' and F.ID = '{this.txtfactory.Text}' ");
