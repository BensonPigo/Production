...
";
            ...
            this.comboTransferTo.DataSource = dtTransTo;
            this.comboTransferTo.DisplayMember = "SystemName";
            this.comboTransferTo.ValueMember = "SystemName";
        }
        ...
        protected override void OnFormLoaded()
        {
            ...
            colPackId.CellMouseDoubleClick += (s, e) =>
            {
                ...
                {
                    string packID = this.gridTransferSisterFty.CurrentRow.Cells["ID"].Value.ToString();
...
";
            ...
            // Grid設定
            this.gridTransferSisterFty.IsEditingReadOnly = false;
            this.gridTransferSisterFty.DataSource = this.listControlBindingSource1;
            this.Helper.Controls.Grid.Generator(this.gridTransferSisterFty)
                .Date("PLCtnTrToRgCodeDate", header: "Transfer Date", iseditingreadonly: true)
                .Text("ID", header: "Pack ID", width: Widths.Auto(), iseditingreadonly: true, settings: colPackId)
        ...
        private void Find()
        {
            ...
            if (this.dateTransfer.HasValue1)
            {
                sqlWhere += " and p.PLCtnTrToRgCodeDate  >= @PLCtnTrToRgCodeDateFrom";
                listPar.Add(new SqlParameter("@PLCtnTrToRgCodeDateFrom", this.dateTransfer.Value1));
            }
...
where   p.MDivisionID = '{Env.User.Keyword}' and
        p.Type = 'B' and
        (pu.Status = 'New' or pu.Status is null) and
        p.PLCtnTrToRgCodeDate is not null and
        o.FactoryID in (select F.ID from Factory F where F.Junk = 0 and F.IsProduceFty = 1 and F.NegoRegion = '{this.comboTransferTo.Text}')
        {sqlWhere}
...
";
            ...
            foreach (var item in listResultGroup)
            {
                ...
                dr["PLCtnTrToRgCodeDate"] = item.First()["PLCtnTrToRgCodeDate"];

                dtResultGroup.Rows.Add(dr);
            }

            this.gridTransferSisterFty.DataSource = dtResultGroup;
        }
        ...
        {
            string sqlPopup = $@"select F.ID from Factory F 
where F.Junk = 0 and F.IsProduceFty = 1 and F.NegoRegion = '{this.comboTransferTo.Text}'
";
        ...
        private void TxtFactory_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            string sqlCheck = $@"select 1 from Factory F 
where   F.Junk = 0 and 
        F.IsProduceFty = 1 and 
        F.NegoRegion = '{this.comboTransferTo.Text}' and
        F.ID = '{this.txtFactory.Text}'
";
