           ...
           if (radioButton_MNotice.Checked == true)
            {
              ...
              string poid =   MyUtility.GetValue.Lookup("select POID FROM dbo.Orders where ID = @ID", new List<SqlParameter> { new SqlParameter("@ID", _id) });
                ...
                string xltPath = Sci.Env.Cfg.XltPathDir + "PPIC_P01_M_Notice.xltx";
            ...
            else
            {
                ...
                System.Data.DataTable[] dts;
                DualResult res = DBProxy.Current.SelectSP("", "PPIC_Report04", new List<SqlParameter> { new SqlParameter("@ID", _id), new SqlParameter("@WithZ", chkAdditional.Checked) }, out dts);

                if (drvar == null | !res) return false;

                string xltPath =Sci.Env.Cfg.XltPathDir + "\\PPIC_P01_M_Notice_Combo.xlt";
                sxrc sxr = new sxrc(xltPath);
                sxr.dicDatas.Add(sxr._v + "Now", DateTime.Now);
                sxr.dicDatas.Add(sxr._v + "MAKER", drvar["MAKER"].ToString());
                sxr.dicDatas.Add(sxr._v + "STYLENO", drvar["sty"].ToString());
                sxr.dicDatas.Add(sxr._v + "QTY", drvar["QTY"].ToString());
                sxr.dicDatas.Add(sxr._v + "SP", drvar["SPNO"].ToString());
                //sxr.dicDatas.Add(sxr._v + "S1_Tbl1", new sxrc.xltRptTable(dts[0], 1, 0, false, 18, 2));
                //For SizeSpec
                sxrc.xltRptTable xltTbl = new sxrc.xltRptTable(dts[0], 1, 0, false, 18, 2);
                for (int i = 3; i <= 18; i++)
                {
                ...
                sxr.dicDatas.Add(sxr._v + "S2_Tbl1", tbl1);
                sxr.dicDatas.Add(sxr._v + "S2_Tbl2", tbl2);
                sxr.dicDatas.Add(sxr._v + "S2_Tbl3", tbl3); //COLOR list                
                ...
                foreach (DataRow dr in dts[9].Rows)
                {
                    ...
                    sxr.dicDatas.Add(sxr._v + "S3_SP" + sIdx, dr["ID"].ToString());
                    sxr.dicDatas.Add(sxr._v + "S3_Style" + sIdx, dr["sty"].ToString());
                    sxr.dicDatas.Add(sxr._v + "S3_QTY" + sIdx, dr["QTY"]);
                    sxr.dicDatas.Add(sxr._v + "S3_CUSTCD" + sIdx, dr["CustCDID"].ToString());
                    ...
                    if (res)
                    {
                        ...
                        sxr.dicDatas.Add(sxr._v + "S3_Tbl" + sIdx, stbl);
                    }
                }
