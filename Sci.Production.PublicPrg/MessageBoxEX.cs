...
namespace Sci.Production.Prg
{
    ...
    public class MessageBoxEX
    {
        ...
        public static DialogResult Show(string text, string caption, MessageBoxButtons buttons, string[] buttonTitles)
        {
            ...
            DialogResult result = MessageBox.Show(frm, text, caption, buttons, MessageBoxIcon.Information);
            frm.Close();
        ...
        private class MessageForm : Form
        {
            ...
            {
                if (this.WatchForActivate && m.Msg == 0x0006)
                {
                    this.WatchForActivate = false;
            ...
            private void CheckMsgbox()
            {
                ...
                IntPtr h = NativeWin32API.GetWindow(this._handle, GW_CHILD);
                ...
                while (h != IntPtr.Zero)
                {
                    ...
                    {
                        if (this._buttonTitles.Length > buttonTitleIndex)
                        {
                            // Changes the text of the specified window's title bar (if it has one). 
                            // If the specified window is a control, the text of the control is changed. 
                            // However, SetWindowText cannot change the text of a control in another application.
                            NativeWin32API.SetWindowText(h, this._buttonTitles[buttonTitleIndex]);
