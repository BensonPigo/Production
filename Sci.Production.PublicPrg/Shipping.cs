...
inner join ShareExpense s on t.AccountID = s.AccountID and t.InvNo = s.InvNo and s.ShippingAPID = @ShippingAPID
...
",
        ...
        /// <summary>
        /// CalculateShareExpense 前半段更新
        /// </summary>
        /// <param name="shippingAPID"></param>
        /// <param name="userID"></param>
        /// <param name="IsFreightFwd"></param>
        /// <returns></returns>
        public static DualResult CalculateShareExpense_ForA2B(string shippingAPID, string userID, int IsFreightFwd)
        {
            DualResult result;
            string sqlcmd = $@"
declare @ShippingAPID varchar(20) ='{shippingAPID}'
...
declare @login varchar(20) = '{userID}'
declare @adddate DATETIME = Getdate()
declare @IsFreightFwd int = {IsFreightFwd}

BEGIN
if len(@ShippingAPID) = 0
	begin
		return
	end 
	...
	select @ShippingReasonID = [Reason] from ShippingAP sp where sp.ID = @ShippingAPID
