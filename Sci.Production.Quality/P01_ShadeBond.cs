...
,Inspector,Remark from FIR_Shadebone WITH (NOLOCK) where id='{0}' AND Result!= '' ", this.ID);
        ...
        private void BtnPrintFormatReport4_Click(object sender, EventArgs e)
        {
            // 抓表頭資料
            string cmd = $"select NameEN from Factory WITH (NOLOCK) where id='{Env.User.Factory}'";
            ...
            // 抓Invo,ETA 資料
            cmd = $"select id,Eta from Export where id='{this.displayWKNo.Text}' ";
...
"));
            ...
            List<P01_ShadeBond4_Data> data = dt.AsEnumerable()
                .Select(row1 => new P01_ShadeBond4_Data()
                {
        ...
        {
            string suppid = this.txtsupplier.TextBox1.Text + " - " + this.txtsupplier.DisplayBox1.Text;
            string colorid = this.displayColor.Text;
            string styleid = this.displayStyle.Text;
            var p8 = new P01_PrintFormatReport8(this.maindr, suppid, colorid, styleid);
            p8.ShowDialog();
        }
