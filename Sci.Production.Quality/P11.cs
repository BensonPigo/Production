...
@"select 
    ...
    [Artwork] = Stuff((select distinct concat( ',',mdd.ArtworkTypeID)   from MockupCrocking_Detail_detail mdd where mdd.ReportNo = md.ReportNo FOR XML PATH('')),1,1,''),
    md.Result,
...
masterID);
        ...
        {
            Sci.Production.Quality.P11_Detail callNewDetailForm = new P11_Detail(true, this.CurrentMaintain["ID"].ToString(), string.Empty, null, "New");
        ...
        {
            Sci.Production.Quality.P11_Detail callNewDetailForm = new P11_Detail(true, this.CurrentMaintain["ID"].ToString(), this.CurrentDetailData["ReportNo"].ToString(), null, "Edit");
            callNewDetailForm.ShowDialog(this);
            callNewDetailForm.Dispose();
            this.RenewData();
            this.OnDetailEntered();
        ...
        private void DeleteThisDetail()
        {
            ...
            {
                string delelete_cmd = $@"delete MockupCrocking_Detail where ReportNo = '{this.CurrentDetailData["ReportNo"].ToString()}';
                                        delete MockupCrocking_Detail_Detail where ReportNo = '{this.CurrentDetailData["ReportNo"].ToString()}';";
        ...
        protected override bool OnGridSetup()
        {
            ...
            this.detailgrid.CellDoubleClick += (s, e) =>
            {
                ...
                Sci.Production.Quality.P11_Detail callNewDetailForm = new P11_Detail(false, this.CurrentMaintain["ID"].ToString(), this.CurrentDetailData["ReportNo"].ToString(), null, "Query");
                callNewDetailForm.ShowDialog(this);
                callNewDetailForm.Dispose();
            };
        ...
        {
            DualResult result;
            result = DBProxy.Current.Execute(null, $"delete MockupCrocking_Detail_Detail where ID = '{this.CurrentMaintain["ID"]}'");
            if (!result)
            {
                this.ShowErr(result);
            }
