...
namespace Sci.Production.Quality
{
    public partial class R06 : Sci.Win.Tems.PrintForm
    {
        ...
        inner join FIR_Physical_Defect fpd WITH (NOLOCK) on fpd.FIR_PhysicalDetailUKey=fp.DetailUkey
        " + sqlWhere + @"
	) 
	select SID.* ,
		ref.*,
	...
	outer apply(
		select (
			...
			from (SELECT DISTINCT PSD.Refno 
					FROM tmpAllData , dbo.PO_Supp_Detail PSD WITH (NOLOCK) 
	...
	order by Refno
       ...
       select  Tmp.SuppID
       	   ...
       	   ,SHINGKAGE[SHRINKAGE (%)] 
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= SHINGKAGE and range2 >= SHINGKAGE)SHINGKAGELevel
       	   ,MIGRATION.MIGRATIONyards
       	   ,MIGRATION[MIGRATION (%)]
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= MIGRATION and range2 >= MIGRATION)MIGRATIONLevel
       	   ,SHADING.SHADINGyards
       	   ,SHADING[SHADING (%)]
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= SHADING and range2 >= SHADING)SHADINGLevel
       	   ,LACKINGYARDAGE.ActualYds
       	   ,LACKINGYARDAGELevel.LACKINGYARDAGE[LACKINGYARDAGE(%)]
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= LACKINGYARDAGELevel.LACKINGYARDAGE and range2 >= LACKINGYARDAGELevel.LACKINGYARDAGE)LACKINGYARDAGELevel
       	   ,SHORTyards.SHORTWIDTH
       	   ,SHORTWIDTHLevel.SHORTWIDTH[SHORT WIDTH (%)]
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= SHORTWIDTHLevel.SHORTWIDTH and range2 >= SHORTWIDTHLevel.SHORTWIDTH)SHORTWIDTHLevel
       	   ,TOTAL.avg
       	   ,(select id from SuppLevel WITH (NOLOCK) where type='F' and range1 <= TOTAL.avg and range2 >= TOTAL.avg)TOTALLEVEL
       from (select SuppID
       	        ,refno
       	        ,abben
       	        ,stockqty
              ...
              from  #tmp tmp
              outer apply(select sl.id from SuppLevel sl WITH (NOLOCK) where type='F' and range1 <= [Fabric(%)] and range2 >= [Fabric(%)])FABRIC_Level
              )Tmp
       ...
       outer apply(select Sum(rd.stockqty)SHADINGyards
       			from Receiving_Detail rd WITH (NOLOCK) 
                   Where exists(select * from fir f WITH (NOLOCK) 
                       ...
                       or exists(select * from fir f WITH (NOLOCK) 
       ...
       outer apply(select sum(fp.ActualYds)ActualYds from fir f WITH (NOLOCK) 
       			inner join FIR_Physical fp WITH (NOLOCK) on fp.ID = f.ID 
