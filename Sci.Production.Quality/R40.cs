            ...
            else
            {
                ...
                string sqcmd = string.Format(@"declare @dRanges table(starts int , ends int, name varchar(3))
                   insert into @dRanges values
                    ...
                    where year in (@y1,@y2,@y3)
                    group by Target,Claimed.Claimed,sh.qty,Claimed.month1,Claimed.YEAR1,Claimed.factory
                    select dRanges.name,dRanges.starts,#temp.Target,ISNULL(SUM(year1.Claimed),0)[Claimed1],ISNULL(SUM(year1.Shipped),0)[Shipped1],ISNULL(SUM(year2.Claimed),0)[Claimed2],ISNULL(SUM(year2.Shipped),0)[Shipped2],ISNULL(SUM(year3.Claimed),0)[Claimed3],ISNULL(SUM(year3.Shipped),0)[Shipped3],#temp.factory  from dbo.#temp
                    inner join @dRanges as dRanges on dRanges.starts between dRanges.starts and dRanges.ends 
					...
					GROUP BY dRanges.name,#temp.Target,factory1.Claimed,factory1.Shipped,factory2.Claimed,factory2.Shipped,factory3.Claimed,factory3.Shipped,#temp.factory,dRanges.starts
					order by dRanges.starts
                    DROP TABLE #temp");
                ...
                for (int colIdx = startIndex1; colIdx < dat.Columns.Count; colIdx++)
                {
                    ...
                    {
                        TTColumnAMT1 += Convert.ToDecimal(dat.Rows[rowIdx][colIdx]);
                    }
