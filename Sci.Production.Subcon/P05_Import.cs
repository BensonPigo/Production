...
");
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                foreach (DataRow tmp in dr2)
                {
                    ...
                    DataRow[] findrow = this.dt_artworkReqDetail.Select($@"orderid = '{tmp["orderID"]}' and ArtworkId = '{tmp["ArtworkId"]}' and patterncode = '{tmp["patterncode"]}' and PatternDesc = '{tmp["PatternDesc"]}'");
        ...
        private string QuoteIsSintexSubcon(string sqlWhere)
        {
            ...
            if (this.dt_artworkReqDetail.Rows.Count > 0)
            {
                tmpTable = $@"
outer apply(
    select value = sum(ReqQty) 
    from #tmp
    where OrderID = o.id and ArtworkID = iif(oa.ArtworkID is null,'{this.dr_artworkReq["artworktypeid"]}',oa.ArtworkID)
...
inner join dbo.Order_Artwork oa on oa.ID = o.ID and oa.ArtworkTypeID = '{this.dr_artworkReq["artworktypeid"]}'
...
left join Style_Artwork_Quot sao with (nolock) on   sao.Ukey = vsa.StyleArtworkUkey and 
                                                    sao.LocalSuppId = '{this.dr_artworkReq["LocalSuppId"]}'
inner join factory f WITH (NOLOCK) on o.factoryid=f.id
outer apply (
        select value = ISNULL(sum(ReqQty),0)
        from ArtworkReq_Detail AD, ArtworkReq a
...
outer apply(
    select value = sum(ReqQty) 
    from #tmp
    where OrderID = o.id and ArtworkID = '{this.dr_artworkReq["artworktypeid"]}'
