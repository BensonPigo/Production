    ...
    public partial class R26 : Sci.Win.Tems.PrintForm
    {
        ...
        protected override Ict.DualResult OnAsyncDataLoad(Win.ReportEventArgs e)
        {
          //  List<SqlParameter> lis = new List<SqlParameter>();
            ...
            {
            string sqlcmd1 = string.Format(@"select  a.FactoryID [Factory]
            ...
            result = DBProxy.Current.Select("", sqlcmd1, lis, out dtt);
            if (!result)
            {
                return result; ;
            }
            ...
            else if (this.Report_Type == "PO Order")
            {
                string cmd = string.Format(@"select   b.id [LocalPOID]
                                                 ...
                                                 ,c.APQty [AP_Qty]
                                       from dbo. LocalPO b
                                       left join dbo.LocalPO_Detail c on b.id=c.Id " + sqlWhere);
            ...
            if (Category.TrimEnd().Equals("CARTON", StringComparison.OrdinalIgnoreCase) && checkBox1.Checked == true)
            {
                string scmd = string.Format(@"select a.id 
                                                    ...
                                                    ,a.Markright [right]
                                             from orders a
                                             left join (select distinct OrderId from LocalPO_Detail) m  on m.OrderId = a.poid 
        ...
        protected override bool OnToExcel(ReportDefinition report)
        {
            ...
            if ("PO List".EqualString(this.Report_Type))
            {
        ...
        private void txtartworktype_fty1_TextChanged(object sender, EventArgs e)
        {
            ...
            //txtartworktype_fty1.Text.TrimEnd().Equals("CARTON", StringComparison.OrdinalIgnoreCase);
        }
