...
{
    public partial class P71 : Sci.Win.Tems.Input6
    {
            ...
            : base(menuitem)
        {
            ...
            this.DefaultFilter = string.Format("Type='B' and ToMDivisionID = '{0}'", Sci.Env.User.Keyword);
        ...
        protected override bool ClickSaveBefore()
        {
            ...
            {
                string tmpId = Sci.MyUtility.GetValue.GetID(Sci.Env.User.Keyword + "XB", "RequestCrossM", (DateTime)CurrentMaintain["Issuedate"]);
        ...
        protected override void OnDetailGridSetup()
        {
            ...
            .Text("topoid", header: "Bulk" + Environment.NewLine + "SP#", width: Widths.AnsiChars(13), iseditingreadonly: true)
            .Text("toseq", header: "Bulk" + Environment.NewLine + " Seq", width: Widths.AnsiChars(6), iseditingreadonly: true)
            .EditText("Description", header: "Description", width: Widths.AnsiChars(20), iseditingreadonly: true)
            .Text("frompoid", header: "Inventory" + Environment.NewLine + "SP#", width: Widths.AnsiChars(13), iseditingreadonly: true)
            .Text("fromseq", header: "Inventory" + Environment.NewLine + "Seq", width: Widths.AnsiChars(6), iseditingreadonly: true)
...
Where a.id = '{0}'", masterID);
        ...
        //delete all
        private void ClearQtyIsEmpty_Click(object sender, EventArgs e)
        {
        ...
        {
            CurrentMaintain["mdivisionid"] = txtMdivision.Text.ToString();
            if (MyUtility.Check.Empty(CurrentMaintain["mdivisionid"]))
            {
                MyUtility.Msg.WarningBox("< From M > can't be empty!!");
                return;
            }
            var frm = new Sci.Production.Warehouse.P71_Import(CurrentMaintain, (DataTable)detailgridbs.DataSource);
            frm.ShowDialog(this);
            this.RenewData();
        }
        protected override bool ClickPrint()
        {
            ...
            string sqlcmd = @"
            select RCD.ToPOID,
	               RCD.ToSeq1+'-'+RCD.ToSeq2 AS BulkSEQ,
            ...
            // 傳 list 資料            
            List<P71_PrintData> data = dt.AsEnumerable()
                .Select(row1 => new P71_PrintData()
                {
                    SPNo = row1["ToPOID"].ToString().Trim(),
                    BulkSeq = row1["BulkSEQ"].ToString().Trim(),
                    DESC = row1["description"].ToString().Replace(Environment.NewLine, "").Trim(),
                    InventorySP = row1["FromPOID"].ToString().Trim(),
                    InventorySeq = row1["InventorySEQ"].ToString().Trim(),
