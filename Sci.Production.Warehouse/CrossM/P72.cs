...
{
    public partial class P72 : Sci.Win.Tems.Input6
    {
        ...
        protected override bool ClickSaveBefore()
        {
            ...
            {
                string tmpId = Sci.MyUtility.GetValue.GetID(Sci.Env.User.Keyword + "XC", "Issue", (DateTime)CurrentMaintain["Issuedate"]);
...
where (isnull(f.InQty,0)-isnull(f.OutQty,0)+isnull(f.AdjustQty,0) - d.Qty < 0) and d.Id = '{0}'", CurrentMaintain["id"]);
        ...
        protected override void ClickUnconfirm()
        {
            ...
            string P73_status = MyUtility.GetValue.Lookup(string.Format(@"select Status from RequestCrossM WITH (NOLOCK) where ID='{0}'", CurrentMaintain["CutplanID"]));
...
Where a.id = '{0}'", masterID);
        ...
        protected override bool ClickPrint()
        {
            ...
            pars.Add(new SqlParameter("@ID", id));
            string sqlcmd = @"select issd.poid,
       ...
       left join dbo.PO_Supp_Detail p WITH (NOLOCK) on p.id= issd.poid and p.SEQ1 = issd.Seq1 and p.seq2 = issd.Seq2
       where issd.id = @ID";
            DualResult res;
            ...
            // 傳 list 資料            
            List<P72_PrintData> data = dt.AsEnumerable()
                .Select(row1 => new P72_PrintData()
                {
                    ...
                    IssueQTY = row1["Qty"].ToString().Trim(),
            ...
            // 指定是哪個 RDLC
            //DualResult result;
            Type ReportResourceNamespace = typeof(P72_PrintData);
            Assembly ReportResourceAssembly = ReportResourceNamespace.Assembly;
            string ReportResourceName = "P72.rdlc";
