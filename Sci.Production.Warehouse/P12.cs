...
where (isnull(f.InQty,0) - isnull(f.OutQty,0) + isnull(f.AdjustQty,0) - isnull(f.ReturnQty,0) - d.Qty < 0) and d.Id = '{0}'", this.CurrentMaintain["id"]);
            ...
            if (Vstrong_AutoWHAccessory.IsVstrong_AutoWHAccessoryEnable)
            {
                ...
                Task.Run(() => new Vstrong_AutoWHAccessory().SentIssue_Detail_New(dtDetail, "P12", "New"))
...
where (isnull(f.InQty,0) - isnull(f.OutQty,0) + isnull(f.AdjustQty,0) - isnull(f.ReturnQty,0) + d.Qty < 0) and d.Id = '{0}'", this.CurrentMaintain["id"]);
            ...
            {
                DataTable dtDetail = this.CurrentMaintain.Table.AsEnumerable().Where(s => s["ID"] == this.CurrentMaintain["ID"]).CopyToDataTable();
                if (!Vstrong_AutoWHAccessory.SentIssue_Detail_delete(dtDetail, "P12", "UnConfirmed"))
                {
                    return;
                }
            }
...
Where a.id = '{0}'", masterID);
        ...
        protected override bool ClickPrint()
        {
            ...
            string confirmTime = row["Status"].EqualString("CONFIRMED") ? MyUtility.Convert.GetDate(row["EditDate"]).Value.ToString("yyyy/MM/dd HH:mm:ss") : string.Empty;
...
where a.id= @ID";
            result = DBProxy.Current.Select(string.Empty, cmdd, pars, out dtt);
            if (!result)
            {
                this.ShowErr(result);
            }
        ...
        {
            P99_CallForm.CallForm(this.CurrentMaintain["ID"].ToString(), "P12", this);
        }
    }
}