...
namespace Sci.Production.Warehouse
{
    public partial class P22 : Sci.Win.Tems.Input6
    {
        ...
        protected override bool ClickSaveBefore()
        {
            ...
            {
                string tmpId = Sci.MyUtility.GetValue.GetID(Sci.Env.User.Keyword + "ST", "SubTransfer", (DateTime)this.CurrentMaintain["Issuedate"]);
...
where (isnull(f.InQty,0)-isnull(f.OutQty,0)+isnull(f.AdjustQty,0) + d.Qty < 0) and d.Qty<0  and d.Id = '{0}'", this.CurrentMaintain["id"]);
            if (!(result2 = DBProxy.Current.Select(null, sqlcmd, out datacheck)))
            {
                this.ShowErr(sqlcmd, result2);
                return;
...
Where a.id = '{0}'", masterID);
        ...
        {
            var frm = new Sci.Production.Warehouse.P22_Import(this.CurrentMaintain, (DataTable)this.detailgridbs.DataSource);
            frm.ShowDialog(this);
            this.RenewData();
        }
        ...
        {
            var frm = new Sci.Production.Warehouse.P22_AccumulatedQty(this.CurrentMaintain);
            frm.P22 = this;
            frm.ShowDialog(this);
        }
