...
{
    /// <inheritdoc/>
    public partial class P33_Thread_Detail : Sci.Win.Subs.Base
    {
        private DataRow style;
        private DataRow style_ThreadColorCombo_History;
        private DataTable gridTable;
        private DataTable tbArticle;
        private string combdetail_id;

        /// <inheritdoc/>
        public P33_Thread_Detail(DataRow dr_Style_ThreadColorCombo_History)
        {
            this.InitializeComponent();
            this.style_ThreadColorCombo_History = dr_Style_ThreadColorCombo_History;
            this.combdetail_id = dr_Style_ThreadColorCombo_History["Ukey"].ToString();

            DataTable dt;
            string cmd = $@"
SELECT *
FROM Style
WHERE Ukey='{MyUtility.Convert.GetString(dr_Style_ThreadColorCombo_History["StyleUkey"])}'
";
            ...
            this.displayThreadCombination.Text = MyUtility.Convert.GetString(dr_Style_ThreadColorCombo_History["Thread_ComboID"]);
            this.displayMachineType.Text = MyUtility.Convert.GetString(dr_Style_ThreadColorCombo_History["MachineTypeID"]);
        }
        ...
        private void GenerateGrid() // 建立Gird
        {
            ...
            else
            {
                ...
                {
                    art_col.Append($@",[{dr["article"].ToString().Trim()}]");
                    art_col.Append($@",[{dr["article"].ToString().Trim()}_Brand]");
                }
            }
            #endregion

            #region gridDetail 設定
            ...
            this.Helper.Controls.Grid.Generator(this.gridDetail)
                ...
                .Text("UseRatio", header: "UseRatioNumeric", width: Widths.Auto(true), iseditingreadonly: true)
            ...
            // 前4個是MachineType_ThreadRatio,後一個Refno是Style_ThreadColorCombo_Detail
...
outer apply (
	...
	from Style_ThreadColorCombo_History_Detail t WITH (NOLOCK) 
	where SEQ = TD.SEQ
	and Article='{this.tbArticle.Rows[i][0].ToString().Trim()}'
    and t.Style_ThreadColorCombo_HistoryUkey ='{this.combdetail_id}'
...
select
    ...
    TD.UseRatio,
    TD.Allowance,
    TD.Style_ThreadColorCombo_HistoryUkey as Style_ThreadColorComboUkey,
    TD.SCIRefNo,
	TD.SuppId,
    TD.SuppName
    {art_col}
from (
	select t.Seq, t.SCIRefNo, t.Style_ThreadColorCombo_HistoryUkey, t.SuppId, iif(Supp.AbbEN = '', Supp.AbbCH, Supp.AbbEN) as SuppName,t.UseRatio,t.Allowance
	from Style_ThreadColorCombo_History_Detail t WITH (NOLOCK) 
    left join Supp on t.SuppId = Supp.Id
	where t.Style_ThreadColorCombo_HistoryUkey ='{this.combdetail_id}'
    and t.Article in (select Article from Style_Article where StyleUkey = {this.style["Ukey"]})
    group by t.Seq, t.SCIRefNo,t.Style_ThreadColorCombo_HistoryUkey, t.SuppId, Supp.AbbCH, Supp.AbbEN, t.UseRatio, t.Allowance
)TD
Left Join MachineType_ThreadRatio MT WITH (NOLOCK) on MT.ID='{this.style_ThreadColorCombo_History["MachinetypeID"]}' and MT.Seq = TD.Seq	
{op}
";
