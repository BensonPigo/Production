...
where (isnull(f.InQty,0) - isnull(f.OutQty,0) + isnull(f.AdjustQty,0) - d.Qty < 0) and d.Id = '{0}'", this.CurrentMaintain["id"]);
 ...
 , [Location] = s.Location
 from LocalOrderIssue_Detail td
 left join Production.dbo.LocalOrderInventory f on f.POID = td.POID 
            ...
            if (MyUtility.Check.Seek(@"select * from System where WH_MtlTransChkLocation = 1"))
            {
                if (dtLocationDetail != null && dtLocationDetail.Rows.Count > 0)
                {
                    ...
                    if (dtArry != null && dtArry.Length > 0)
                    {
                        ...
                        Prgs.ChkLocationEmpty(dtLocation_Empty, "LocalOrder", @"SP#,Seq,Roll,Dyelot,Stock Type");
                        return;
                    }
                }
            }
...
where (isnull(f.InQty,0) - isnull(f.OutQty,0) + isnull(f.AdjustQty,0) + d.Qty < 0) and d.Id = '{0}'", this.CurrentMaintain["id"]);
        ...
        protected override bool ClickSaveBefore()
        {
            ...
            {
                this.CurrentMaintain["ID"] = MyUtility.GetValue.GetID(Env.User.Keyword + "OI", "LocalOrderIssue", (DateTime)this.CurrentMaintain["AddDate"]);
            }
...
select  loid.*,
	    ...
	    [Total]=sum(loid.Qty) OVER (PARTITION BY loid.POID ,loid.seq)
from    LocalOrderIssue_Detail loid
